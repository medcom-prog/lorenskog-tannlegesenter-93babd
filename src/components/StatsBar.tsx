...
const stats = [
  ...
  { value: '690 kr', label: 'akuttundersøkelse', detail: 'prisoverslag gis alltid før start' },
  { value: 'Helfo', label: 'dekker odontofobi', detail: 'store deler av kostnadene' },
];

export function StatsBar() {
  return (
    <section className="bg-[#1A1008] py-10" aria-label="Nøkkelfakta">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8">
          {stats.map((stat, i) => (
            ...
            >
              ...
              {/* #B8A090 on #1A1008 = sufficient contrast */}
              <p className="text-[#B8A090] text-xs mt-0.5">{stat.detail}</p>
            </motion.div>
          ))}
        </div>
      </div>
